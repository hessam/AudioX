      ...
      <div className="flex-1 overflow-y-auto bg-[#1a1a1a] custom-scrollbar">
        {activeTab === 'info' && (
          <div className="p-4 sm:p-6 space-y-6">
            ...
            {/* AUDIO ANALYSIS REPORT (Added for SPS) */}
            {selection.type === 'node' && selection.data.audioAnalysis && (
                <div className="bg-zinc-900 border border-emerald-500/50 rounded-xl p-4 shadow-lg animate-fadeIn">
                    <h3 className="text-emerald-400 font-bold uppercase tracking-widest text-[10px] mb-3 border-b border-emerald-900/50 pb-2 flex items-center gap-2">
                        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19.428 15.428a2 2 0 00-1.022-.547l-2.384-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z" /></svg>
                        Sonic Analysis Report
                    </h3>
                    <div className="space-y-3">
                         <div>
                            <span className="text-gray-500 text-[10px] uppercase font-bold">Detected Genre</span>
                            <div className="text-white font-bold">{selection.data.audioAnalysis.genre}</div>
                         </div>
                         <div>
                            <span className="text-gray-500 text-[10px] uppercase font-bold">Atmosphere / Mood</span>
                            <div className="text-white text-sm">{selection.data.audioAnalysis.mood}</div>
                         </div>
                         <div className="bg-emerald-900/10 p-2 rounded border border-emerald-900/20">
                            <span className="text-emerald-500 text-[9px] uppercase font-bold block mb-1">AI Reasoning</span>
                            <div className="text-gray-300 text-sm italic leading-relaxed">"{selection.data.audioAnalysis.reasoning}"</div>
                         </div>
                    </div>
                </div>
            )}
